...
async function evaluateRule(rule: AlertRule, db: admin.firestore.Firestore, now: admin.firestore.Timestamp) {
...
async function evaluateBudgetThreshold(rule: AlertRule, db: admin.firestore.Firestore) {
...
async function evaluateBillDue(rule: AlertRule, db: admin.firestore.Firestore, now: admin.firestore.Timestamp) {
...
async function evaluateGoalProgress(rule: AlertRule, db: admin.firestore.Firestore, now: admin.firestore.Timestamp) {
  if (!rule.goalId) {
...
async function createNotification(rule: AlertRule, title: string, body: string, db: admin.firestore.Firestore) {
  const notification: Omit<ClientNotification, 'id' | 'createdAt'> & { createdAt: admin.firestore.Timestamp } = {
    userId: rule.userId,
    alertRuleId: rule.id,
    content: { title, body },
    status: {
